...
data class Progress(
     ...
     */
    val percentage: Int
        get() = if (totalBytes > 0) ((bytesTransferred * 100) / totalBytes).toInt().coerceIn(0, 100) else -1

    /**
     * Completion fraction from 0.0f to 1.0f, or -1.0f if [totalBytes] is unknown.
     */
    val fraction: Float
        get() = if (totalBytes > 0) (bytesTransferred.toFloat() / totalBytes.toFloat()).coerceIn(0f, 1f) else -1f
